...
from a_memorix.ports import LLMProvider, LLMRequest, MessageSource
...
class SummaryImporter:
    ...
    def _resolve_summary_model_task(self) -> Optional[Tuple[str, Any]]:
        """
        ...
        return base_task_name, base_cfg

    def _resolve_summary_model_config(self) -> Optional[Any]:
    ...
    async def _import_from_stream_unlocked(
        ...
        try:
            ...
            if self.message_source is None:
                return SummaryImportResult(False, "未配置消息来源")
            messages = await self.message_source.list_messages(
            ...
            # 转换为可读文本
            chat_history_text = "\n".join(
                f"{message.sender_name or message.sender_id or 'unknown'}: {message.content}"
            ...
            if include_personality:
                personality = str(agent_config.get("personality", "") or "")
                if personality:
