...
from a_memorix.ports import IdentityResolver, LLMProvider
...
class PersonProfileService:
    ...
    def resolve_person_id(self, identifier: str) -> str:
        ...
        if self.identity_resolver is None:
            return key
        try:
            record = self.identity_resolver.resolve(key)
        except Exception as exc:
            logger.warning(f"身份解析失败: identifier={key}, err={exc}")
            return ""
        return str(record.person_id or "").strip() if record is not None else ""
    ...
    def get_person_aliases(self, person_id: str) -> Tuple[List[str], str, List[str]]:
        ...
        if self.identity_resolver is not None:
            try:
                record = self.identity_resolver.resolve(person_id)
                external_aliases = [str(item or "").strip() for item in self.identity_resolver.aliases(person_id)]
                if record is not None:
                    primary_name = str(record.display_name or "").strip()
                    external_aliases.extend(record.aliases)
            except Exception as exc:
                logger.warning(f"解析人物别名失败: person_id={person_id}, err={exc}")
        primary_name = primary_name or recovered_primary_name or person_id
        seen = set()
        for item in [*external_aliases, *recovered_aliases]:
    ...
    def _filter_stale_paragraph_evidence(
        ...
        if not bool(self._cfg("feedback_correction.paragraph_hard_filter_enabled", True)):
            return evidence
        paragraph_hashes = [
    ...
    def _resolve_profile_classification_model(self) -> Optional[ResolvedLLMModel]:
        if self.llm_provider is None:
            return None
        try:
            available_tasks = get_text_generation_model_tasks(self.llm_provider)
