...
class RetrievalTuningManager:
    ...
    async def _select_llm_model(self) -> Optional[ResolvedLLMModel]:
        if self.llm_provider is None:
            return None
        try:
            models = get_text_generation_model_tasks(self.llm_provider) or {}
        except Exception:
            return None
        if not models:
            return None
    ...
    async def _generate_nl_queries_with_llm(self, anchors: List[Dict[str, Any]], *, enabled: bool) -> Dict[str, str]:
        if not enabled or self.llm_provider is None or not anchors:
            return {}
        payload = [
            {
    ...
    async def _suggest_profiles_with_llm(
        ...
        *,
        ...
        if not enabled or self.llm_provider is None or max_count <= 0:
            return []
        prompt = (
            "你是检索调参专家。"
            "请基于基础参数与失败摘要，给出最多 "
