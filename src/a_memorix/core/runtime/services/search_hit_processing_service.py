...
class MemorySearchHitProcessingService(KernelServiceBase):
    ...
    def _paragraph_hidden_by_stale_marks(
        ...
        *,
        ...
        if not token or self.metadata_store is None or not feedback_cfg_paragraph_hard_filter_enabled(self.config):
            return False
    ...
    def _filter_episode_hits(self, hits: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        if self.metadata_store is None or not feedback_cfg_episode_query_block_enabled(self.config):
            return hits
