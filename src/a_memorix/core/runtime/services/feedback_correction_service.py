...
class MemoryFeedbackCorrectionService(KernelServiceBase):
    ...
    def _resolve_feedback_related_person_ids(
        ...
        *,
        ...
        for candidate in candidates:
            person_id = self.person_profile_service.resolve_person_id(candidate)
    ...
    def _mark_feedback_stale_paragraphs(
        ...
        *,
        ...
        if self.metadata_store is None or not feedback_cfg_paragraph_mark_enabled(self.config):
            return {}
    ...
    def _enqueue_feedback_episode_rebuilds(
        ...
        *,
        ...
        if self.metadata_store is None or not feedback_cfg_episode_rebuild_enabled(self.config):
            return []
    ...
    def _enqueue_feedback_profile_refreshes(
        ...
        *,
        ...
        if self.metadata_store is None or not feedback_cfg_profile_refresh_enabled(self.config):
            return []
        queued: List[str] = []
    ...
    async def _feedback_correction_reconcile_loop(self) -> None:
        try:
            while not self._background_stopping:
                await asyncio.sleep(feedback_cfg_reconcile_interval_seconds(self.config))
                if self._background_stopping:
                    break
                if self.metadata_store is None or not feedback_cfg_enabled(self.config):
                    continue
                batch_size = feedback_cfg_reconcile_batch_size(self.config)
                if feedback_cfg_profile_refresh_enabled(self.config):
    ...
    async def enqueue_feedback_task(
        ...
        *,
        ...
        if not feedback_cfg_enabled(self.config):
        ...
        due_at = query_time + timedelta(hours=feedback_cfg_window_hours(self.config))
    ...
    async def _extract_feedback_messages(
        self,
        *,
        session_id: str,
        query_time: datetime,
        ...
        if self.message_source is None:
            return []
        raw_messages = await self.message_source.list_messages(
    ...
    async def _classify_feedback(
        ...
        *,
        ...
        try:
            ...
            payload = safe_json_loads(response.content if response.success else "")
        except Exception as exc:
            ...
            relation_rows.append(
                {
                    ...
                    "metadata": {
                        ...
                        "feedback_window": feedback_cfg_window_label(self.config),
                    },
                }
            )
        plain_text = "；".join(
    ...
    async def _apply_feedback_decision(
        ...
        *,
        ...
        后续人工回退使用的补偿计划。
        """
        threshold = feedback_cfg_auto_apply_threshold(self.config)
    ...
    async def _process_feedback_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            ...
            due_at = coerce_datetime(task.get("due_at")) or (
                query_timestamp + timedelta(hours=feedback_cfg_window_hours(self.config))
            )
            if due_at <= query_timestamp:
                due_at = query_timestamp + timedelta(hours=feedback_cfg_window_hours(self.config))

            feedback_messages = await self._extract_feedback_messages(
                session_id=session_id,
                query_time=query_timestamp,
                due_time=due_at,
                max_messages=feedback_cfg_max_messages(self.config),
            )
            if not feedback_messages:
            ...
            if feedback_cfg_prefilter_enabled(self.config) and not self._should_invoke_feedback_classifier(feedback_messages):
    ...
    async def _feedback_correction_loop(self) -> None:
        try:
            while not self._background_stopping:
                interval_seconds = feedback_cfg_check_interval_seconds(self.config)
                if not feedback_cfg_enabled(self.config):
                ...
                tasks = self.metadata_store.fetch_due_feedback_tasks(
                    limit=feedback_cfg_batch_size(self.config),
