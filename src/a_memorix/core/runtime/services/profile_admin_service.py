...
class MemoryProfileAdminService(KernelServiceBase):
    ...
    async def _query_person_profile_with_feedback_refresh(
        ...
        *,
        ...
        if (
            pid
            and feedback_cfg_profile_refresh_enabled(self.config)
            and feedback_cfg_profile_force_refresh_on_read(self.config)
        ...
        if act == "process_pending":
            result = await self._process_feedback_profile_refresh_batch(
                limit=max(
                    1,
                    int(
                        kwargs.get("limit", feedback_cfg_reconcile_batch_size(self.config)) or feedback_cfg_reconcile_batch_size(self.config)
                    ),
                )
            )
