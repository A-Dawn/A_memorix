...
class MemoryCorrectionAdminService(KernelServiceBase):
    async def memory_correction_admin(self, *, action: str, **kwargs) -> Dict[str, Any]:
        ...
        if act in {"preview", "plan"}:
            return await self._preview_fuzzy_modify_action(
                ...
                limit=max(
                    1,
                    int(kwargs.get("limit", fuzzy_modify_cfg_candidate_limit(self.config)) or fuzzy_modify_cfg_candidate_limit(self.config)),
                ),
    ...
    async def _preview_fuzzy_modify_action(
        ...
        *,
        ...
        assert self.metadata_store is not None
        if not fuzzy_modify_cfg_enabled(self.config):
        ...
        elif not chat_id and not fuzzy_modify_cfg_allow_global_scope(self.config):
            return {"success": False, "error": "非人物画像修正需要提供 chat_id，或开启全局记忆修正范围"}

        candidate_limit = min(max(1, int(limit or 20)), fuzzy_modify_cfg_candidate_limit(self.config))
        ...
        preview = {
            ...
            "confirm_threshold": fuzzy_modify_cfg_confirm_threshold(self.config),
    ...
    async def _execute_fuzzy_modify_action(
        ...
        *,
        ...
        if not confirmed:
            ...
            if not fuzzy_modify_cfg_auto_execute_enabled(self.config) or confidence < fuzzy_modify_cfg_confirm_threshold(self.config):
    ...
    async def _build_fuzzy_modify_llm_plan(
        ...
        *,
        ...
        payload = {
            ...
            "person_keyword": person_keyword,
            "chat_id": chat_id,
            "max_targets": fuzzy_modify_cfg_max_targets(self.config),
            "candidates": [
                {
        ...
        prompt = (
            "根据请求和候选记忆生成修正计划。只输出 JSON，操作仅能引用候选集中的对象。\n"
        ...
        return safe_json_loads(response.content if response.success else "")
