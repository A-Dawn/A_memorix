...
class SDKMemoryKernel(KernelCompatibilityMixin):
    ...
    def _feedback_cfg_enabled(self) -> bool:
        return feedback_cfg_enabled(self.config)

    def _feedback_cfg_window_hours(self) -> float:
        return feedback_cfg_window_hours(self.config)

    def _feedback_cfg_check_interval_seconds(self) -> float:
        return feedback_cfg_check_interval_seconds(self.config)

    def _feedback_cfg_batch_size(self) -> int:
        return feedback_cfg_batch_size(self.config)

    def _feedback_cfg_auto_apply_threshold(self) -> float:
        return feedback_cfg_auto_apply_threshold(self.config)

    def _feedback_cfg_max_messages(self) -> int:
        return feedback_cfg_max_messages(self.config)

    def _feedback_cfg_prefilter_enabled(self) -> bool:
        return feedback_cfg_prefilter_enabled(self.config)

    def _feedback_cfg_paragraph_mark_enabled(self) -> bool:
        return feedback_cfg_paragraph_mark_enabled(self.config)

    def _feedback_cfg_paragraph_hard_filter_enabled(self) -> bool:
        return feedback_cfg_paragraph_hard_filter_enabled(self.config)

    def _feedback_cfg_profile_refresh_enabled(self) -> bool:
        return feedback_cfg_profile_refresh_enabled(self.config)

    def _feedback_cfg_profile_force_refresh_on_read(self) -> bool:
        return feedback_cfg_profile_force_refresh_on_read(self.config)

    def _feedback_cfg_episode_rebuild_enabled(self) -> bool:
        return feedback_cfg_episode_rebuild_enabled(self.config)

    def _feedback_cfg_episode_query_block_enabled(self) -> bool:
        return feedback_cfg_episode_query_block_enabled(self.config)

    def _feedback_cfg_reconcile_interval_seconds(self) -> float:
        return feedback_cfg_reconcile_interval_seconds(self.config)

    def _feedback_cfg_reconcile_batch_size(self) -> int:
        return feedback_cfg_reconcile_batch_size(self.config)
    ...
    def _feedback_cfg_window_label(self) -> str:
        return feedback_cfg_window_label(self.config)
