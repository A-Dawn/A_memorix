...
**轻量级知识图谱插件** - 基于双路检索的完全独立的记忆增强系统 (v0.3.2)
...
> [!NOTE]
> **v0.3.2 稳定性补丁**：
> 1. 修复 V5 记忆系统多处契约不一致问题（冷冻/修剪/恢复链路与后台任务生命周期）；
> 2. 修复 `/api/memory/restore`、回收站查询与 `/memory` hash 判定兼容性；
> 3. 修复 `summarization.model_name=auto` 错误命中 `embedding` 的问题，并支持数组/选择器配置；
> 4. 修复 `process_knowledge.py` 重试日志等级类型错误导致的 `KeyError: 'WARNING'`。
