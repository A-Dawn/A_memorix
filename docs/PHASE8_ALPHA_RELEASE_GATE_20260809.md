...
日期：2026-08-09  
候选版本：2.0.0a2  
状态：Alpha 发布门禁已通过，PyPI外部配置和对外发布待完成

## 已通过门禁

通用分支`refactor/generic-v2`完成验证后已无冲突 fast-forward进入`main`。默认分支在 GitHub Actions 完成 Linux、Windows、Faiss、无 Faiss降级、Python 3.12、Ruff、mypy、679项 Python 测试、Protobuf生成、Buf lint、Go测试、Wheel、sdist、twine检查以及服务、网关双镜像构建。2项可选大规模迁移压测按设计跳过。
...
`v*`发布标签受独立规则保护，创建后不能删除或改写。GitHub的`release`环境已经建立，发布工作流仍要求标签提交属于`main`历史。

## 发布前外部配置

仍需在 PyPI为`A-Dawn/A_memorix`的`release.yml`和`release`环境配置 Trusted Publisher。发布`v2.0.0a2`会对外推送 PyPI包、两个 GHCR镜像、多平台网关二进制和 GitHub Release，因此配置完成前不创建标签。
