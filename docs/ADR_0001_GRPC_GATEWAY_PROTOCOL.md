...
v1 覆盖 namespace 生命周期、配置与能力发现、API Key 生命周期、单条和批量文本写入、记忆检索、直接读取、单条删除，以及按来源删除 Job。Episode、画像、摘要等已有内核能力尚未进入公开 Protobuf API，它们需要先完成领域语义通用化。

写入幂等账本持久化已完成响应，并校验同一幂等键对应的请求摘要。正常完成的请求可以跨服务重启重放原响应。元数据、向量库和图存储不是单一事务，因此进程在业务提交与账本提交之间异常退出时，重试依靠 external ID 去重，不承诺跨存储的全局 exactly-once。
