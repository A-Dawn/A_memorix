...
阶段5把已经完成的通用内核、namespace 控制面和统一协议整理成可安装、可启动、可诊断、可发布的服务。核心协议仍是 Protobuf 定义的 gRPC，HTTP/JSON 只由 gRPC-Gateway 映射。运维入口不得绕过 RPC 直接修改服务数据目录。

Episode、画像、摘要等领域能力的通用语义不在本阶段强行冻结。它们继续保留在内核中，等待后续阶段按 Agent 无关的 contract 设计。
...
密钥值不进入配置模型。管理员令牌和 namespace API Key 通过环境变量或只读文件加载，`config` 与 `doctor` 的输出只包含令牌文件路径，不包含令牌内容。参考配置位于`deploy/a-memorix.example.toml`。
...
日志默认是单行 JSON，记录 UTC 时间、级别、logger、RPC 方法、gRPC 状态和处理耗时。可选 Prometheus 端点暴露请求计数、耗时直方图和活跃请求数；可选 OTLP/gRPC 导出 RPC Trace。可观测性依赖保留在独立 extra 中，不增加基础内核安装负担。
...
阶段6开始建立官方 Agent 集成分支。该阶段应复用当前公开 contract、CLI 和镜像，不把 MaiBot 配置对象或数据库类型带回通用主线，也不借集成工作重新设计已经稳定的通用边界。
