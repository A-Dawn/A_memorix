...
阶段0基线中有3个真实存储测试稳定失败。共同表现是运行时重启后向量 ID 消失，日志显示完整的 V2 向量世代被判定为 `v2_fingerprint_mismatch`，随后被移动到隔离目录并替换为空世代。

根因不在向量二进制文件。Embedding 适配器在启动时尚未完成真实请求，只能根据候选模型配置生成临时指纹。如果上一次运行实际使用了回退候选模型，临时指纹便可能与磁盘中记录的已观测模型不同。旧状态机把这种尚未确认的兼容性问题当成存储损坏处理，造成不必要的数据隔离。
...
启动校验只接受来源为 `observed` 的模型指纹。没有 `source` 字段的确定性本地适配器仍按已确认指纹处理，以兼容现有受控实现。来源为 `configured` 的指纹不能再触发 V2 向量加载、隔离或覆盖。

真实 embedding 探测现在可以在向量通道关闭时运行。探测完成后：

- 实际模型指纹与原世代一致时，运行时重新加载单池或双池，恢复检索和写入能力。
- 实际模型维度或指纹不一致时，原世代留在原路径，运行时进入 `incompatible` 状态。
- 用户执行 `rebuild_all_vectors` 后，运行时按当前模型重建向量，成功时恢复健康状态和向量能力。
...
修复暂时保留在 MaiBot，作为阶段2迁入通用主线时的已验证来源。通用仓库不直接复制当前 MaiBot 的宿主依赖，迁入时仍需按 Host Port、namespace 和标准包结构拆分。
