...
from a_memorix.core.runtime.services import memory_maintenance_service, memory_search_service
from a_memorix.core.utils.search_execution_service import SearchExecutionResult
...
def test_runtime_lifecycle_close_rejects_initialized_runtime(
    ...
    kernel = SDKMemoryKernel(data_dir=tmp_path, config={})
    kernel.metadata_store = FakeMetadataStore()  # type: ignore[assignment]
    kernel._initialized = True
    monkeypatch.setattr(kernel, "_persist", lambda: events.append("persist"))
...
async def test_data_directory_allows_only_one_active_sdk_writer(
    ...
    first = SDKMemoryKernel(data_dir=tmp_path, config=config)
    second = SDKMemoryKernel(data_dir=tmp_path, config=config)
...
async def test_failed_initialize_discards_writable_references_and_releases_lock(
    ...
    config = {"storage": {"data_dir": str(tmp_path / "shared-memory")}}
    failed = SDKMemoryKernel(data_dir=tmp_path, config=config)
    replacement = SDKMemoryKernel(data_dir=tmp_path, config=config)
...
async def test_persist_failure_invalidates_old_writer_before_unlock(
    ...
    config = {"storage": {"data_dir": str(tmp_path / "shared-memory")}}
    kernel = SDKMemoryKernel(data_dir=tmp_path, config=config)
    successor = SDKMemoryKernel(data_dir=tmp_path, config=config)
    metadata_close_calls = 0
