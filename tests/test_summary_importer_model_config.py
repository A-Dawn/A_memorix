...
from a_memorix.core.utils.summary_importer import (
    SummaryImporter,
    _message_timestamp,
    _normalize_entity_items,
    _normalize_relation_items,
)
from a_memorix.ports import LLMResult


def _task(model: str, *, max_tokens: int = 256, temperature: float = 0.5):
...
def _fake_available_models() -> dict[str, object]:
    return {
        "memory": _task("memory-model", max_tokens=512, temperature=0.4),
        "utils": _task("utils-model"),
        "replyer": _task("replyer-model", max_tokens=128, temperature=0.7),
    }
...
def test_resolve_summary_model_config_auto_falls_back_to_utils_then_planner():
    ...
    importer.llm_provider = _FakeLLMProvider(
        {
            "planner": _task("planner-model"),
            "replyer": _task("replyer-model"),
        }
...
def test_resolve_summary_model_config_auto_does_not_fallback_to_replyer():
    importer = SummaryImporter(
        vector_store=None,
        graph_store=None,
        metadata_store=None,
        ...
        llm_provider=_FakeLLMProvider(
            {
                "replyer": _task("replyer-model"),
                "embedding": _task("embedding-model"),
            }
        ),
    )
...
def test_resolve_summary_model_config_rejects_legacy_string_selector():
    importer = SummaryImporter(
        vector_store=None,
        graph_store=None,
        metadata_store=None,
...
def test_resolve_summary_model_config_skips_task_with_invalid_model():
    importer = SummaryImporter(
        vector_store=None,
        graph_store=None,
        metadata_store=None,
